...
float generateRandomFloat(float min, float max) {
    ...
    return min + (randomNormalized * (max - min));;
}
...
void SimpleCompute::SetupSimplePipeline()
{
  ...
  m_Output = vk_utils::createBuffer(m_device, sizeof(float) * m_length, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                                                                       VK_BUFFER_USAGE_TRANSFER_DST_BIT);                                                                      
  vk_utils::allocateAndBindWithPadding(m_device, m_physicalDevice, {m_Source, m_Output}, 0);
  ...
  m_cpu.resize(m_length);
  for (uint32_t i = 0; i < values.size(); ++i) {
    values[i] = generateRandomFloat(-100.0, 100.0);
    m_cpu[i] = values[i];
  }
...
void SimpleCompute::Execute()
{
  ...
  std::vector<float> Output(m_length, 0);
  sum = 0.0;
  start = std::chrono::high_resolution_clock::now();
  for (int i = 0; i < m_length; ++i) {
    for (int j = -3; j <= 3; ++j) {
      if (i + j < 0 || i + j >= m_length) {
        continue;
      }
      Output[i] += (m_cpu[i + j] / 7.0f);
    }
    sum += Output[i];
  }
